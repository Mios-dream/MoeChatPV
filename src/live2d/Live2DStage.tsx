...
const setMouthOpen = (
  model: import("untitled-pixi-live2d-engine").Live2DModel,
  value: number,
) => {
  const coreModel = model.internalModel.coreModel as unknown as {
...
export const Live2DStage: React.FC<{
  ...
  useLayoutEffect(() => {
    ...
    if (mouthCues && mouthCues.length > 0) {
      ...
      setMouthOpen(model, mouthCues[cueIndex] ?? 0);
    }
    app.render();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        timeRef.current = 0;
        model.update(0);
        setMouthOpen(model, mouthCues?.[0] ?? 0);
        app.render();
