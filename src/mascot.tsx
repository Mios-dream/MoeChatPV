...
export const Mascot: React.FC<{
  ...
  const bob = Math.sin(frame * 0.055) * 10;
  const tilt = Math.sin(frame * 0.03 + 1) * 0.05;
  const happy = mood === "happy" ? Math.abs(Math.sin(frame * 0.12)) * 0.09 : 0;
  const waveRot =
    mood === "wave"
      ? Math.sin(frame * 0.25) * 0.12
      : 0;
  const sleepRot = mood === "sleep" ? -0.04 : 0;
  ...
  return (
    <div
      style={{
        ...
        transform: `translateY(${bob}px) scale(${0.6 + 0.4 * pop}) rotate(${
          tilt + happy + waveRot + sleepRot
        }rad)`,
        ...style,
      }}
    >
      ...
      {mood === "sleep" ? (
        <div
          style={{
            ...
            fontSize: size * 0.16,
            color: COLORS.purple,
            opacity: 0.7 + 0.3 * Math.abs(Math.sin(frame * 0.08)),
            zIndex: 3,
          }}
        >
          Zzz
