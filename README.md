...
> 注：v3 起去掉了花瓣 / 爱心飘落动效；开屏改为台词 + 旋转樱花 + Logo；并集成了项目同款 Live2D 模型。后续版本统一为角色驱动的连续叙事：全片字幕只有智乃台词、无旁白；每个场景都有角色在场；转场统一使用淡入淡出；功能演示之后的场景统一沿用功能演示阶段的白色斜纹 + 星点背景（`StripedStage`）。
