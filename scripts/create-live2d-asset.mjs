...
const normalizeMotion = (response) => {
  ...
  if (!source || typeof source !== "object" || !source.curves || typeof source.curves !== "object") {
    return undefined;
  }
  const fpsValue = Number(source.fps);
  const durationMs = Number(source.duration);
  if (!Number.isFinite(fpsValue) || fpsValue <= 0 || !Number.isFinite(durationMs) || durationMs <= 0) {
    return undefined;
  }
...
const [ttsResponse, motionResponse] = await Promise.all([
